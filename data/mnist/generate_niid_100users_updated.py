...
for i in trange(10):
    _mnist_data = []
    for l in range(mnist.target.shape[0]):
        if int(mnist.target[l]) == i:
            _mnist_data.append(mnist.data[l])
    mnist_data.append(_mnist_data)
...
for user in range(NUM_USERS):
    for j in range(NUM_LABELS):
        l = (user + j) % 10
        X[user] += mnist_data[l][idx[l]:idx[l] + 10]
        y[user] += [j] * 10
        idx[l] += 10
...
for user in trange(NUM_USERS):
    for j in range(NUM_LABELS):  # 4 labels for each users
        ...
        if idx[l] + num_samples < len(mnist_data[l]):
            X[user] += mnist_data[l][idx[l]:idx[l] + num_samples]
            y[user] += [l] * num_samples
            idx[l] += num_samples
...
for i in range(NUM_USERS):
    ...
    train_data['user_data'][uname] = {'x': [j.tolist() for j in X[i][:train_len]],
                                      'y': [j for j in y[i][:train_len]]}
    train_data['num_samples'].append(train_len)
    test_data['users'].append(uname)
    test_data['user_data'][uname] = {'x': [j.tolist() for j in X[i][train_len:]],
                                     'y': [j for j in y[i][train_len:]]}
...
# print(type(train_data['user_data']['f_00000']['x'][0]))
# print(len(train_data['user_data']['f_00000']['x']))
# print(len(train_data['user_data']['f_00000']['x'][0]))
# print(type(train_data['user_data']['f_00000']['y'][0]))
# print(len(train_data['user_data']['f_00000']['y']))
for i in range(10):
    print(i, train_data['user_data']['f_{0:05d}'.format(i)]['y'])
