...
EMBEDDING_PATH = {'shakespeare': './data/shakespeare/data/embs/embs.json'
    , 'sent140': './data/sent140/data/embs/embs.json'
                  }


def read_embedding(name_dataset):
    with open(EMBEDDING_PATH[name_dataset], 'r') as fp:
        embedings = json.load(fp)
    return embedings
...
def get_word_emb_arr(path):
    ...
    for i in range(len(vocab)):
        indd[vocab[i]] = i
    vocab = {i:w for i, w in enumerate(embs['vocab'])}
